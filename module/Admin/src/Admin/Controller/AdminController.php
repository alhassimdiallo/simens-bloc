...
//     	var_dump($passwordDecrypte); exit();
    	...
    	$listeService = $this->getServiceTable ()->fetchService ();
    	$formUtilisateur->get ( 'service' )->setValueOptions ( $listeService );
 		...
 		$request = $this->getRequest();
  		...
  		if ($request->isPost()){
  			...
  			//GESTION DES PHOTOS
  			/*
  			$nomfile = (new \DateTime ( 'now' ))->format ( 'dmy_His' );
  			$date_enregistrement = (new \DateTime ( 'now' ))->format ( 'Y-m-d H:i:s' );
  			...
  			if ($img != false) {
  				...
  				$this->getUtilisateurTable()->modifierPhotoPatient ( $nomfile , $id_employe);
  			} else {
  				$this->getUtilisateurTable()->modifierPhotoPatient ( null , $id_employe);
  			}
  			*/
  			//------------------
  			//------------------
